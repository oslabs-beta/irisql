...
const FieldItem = (props) => {
  ...
  objectRelations.forEach((obj) => {
    objectRelationOptions.push(<option value={obj} key={`obj-${obj.objectName}${1}`}>{obj}</option>);
  });
  ...
  const fieldRelations = objectListState.objects.length && props.info.relatedObjectName
    ? objectListState.objects
      .filter((obj) => obj.objectName === props.info.relatedObjectName)[0].fields : [];
  ...
  fieldRelations.forEach((field) => {
    fieldRelationsOptions.push(<option value={field.fieldName} key={`field-${field.fieldName}${1}`}>{field.fieldName}</option>);
  });
